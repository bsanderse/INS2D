...
switch options.rom.opinf_type
    case {"OpInf", "EC-OpInf Koike", "EC-OpInf skew"}
        ...
        % A_clean = [1 0 2 1 1 0; 0 1 0 1 1 2]; % only for M=2
        % A_dot_clean = conv_quad*vectorwise_kron(A_clean);

        % options.rom.A = A_clean;
        % options.rom.A_dot = A_dot_clean;
        ...
        % not yet implemented
        M = options.rom.M;
        options.rom.yDiff = zeros(M,1);
        options.rom.Conv_linear = zeros(M,M);
        options.rom.yConv       = zeros(M,1);
