function [B,B_inv,M] = getVelocityBasisFDGFourier(options)
...
% avoid building Phi, just transform C:
transform_C = 0;
...
B_inv = 1./diag_mass_matrix; %(abs(mass_matrix)>1e-8); %spdiags(mass_matrix(abs(mass_matrix)>1e-8),0,options.grid.Np,options.grid.Np-1);
