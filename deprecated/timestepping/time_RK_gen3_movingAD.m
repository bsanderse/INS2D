...
for i_RK=1:s_RK
    ...
    % moving force is 'implicit' in time (but does not require the solution
    % of a system as long as it is not depending on the velocity)
    t       = tn + c_RK(i_RK)*dt;   
    ...
    utemp      = dt*ku*A_RK(i_RK,:)' + Omu_inv.*Fx_moving;
    vtemp      = dt*kv*A_RK(i_RK,:)' + Omv_inv.*Fy_moving;    
    ...
    uh      = uhn + utemp - c_RK(i_RK)*dt*Omu_inv.*(Gx*dp + y_px);
    vh      = vhn + vtemp - c_RK(i_RK)*dt*Omv_inv.*(Gy*dp + y_py);
