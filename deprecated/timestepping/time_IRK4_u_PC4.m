...
% solve for U_i
% not energy conserving, only second order
...
while (error_nonlinear > nonlinear_acc && i<nonlinear_maxit)
      ...
      % assemble rhs 1
      f1(1:Nu)        =  Omu.*(uhn-u1) + dt*(a11*ku1 + a12*ku2 - c1*(Gx*pn + y_px_1));                       
      f1(Nu+1:Nu+Nv)  =  Omv.*(vhn-v1) + dt*(a11*kv1 + a12*kv2 - c1*(Gy*pn + y_py_1));
    
      % assemble rhs 2 
      f2(1:Nu)        =  Omu.*(uhn-u2) + dt*(a21*ku1 + a22*ku2 - c2*(Gx*pn + y_px_2));                        
      f2(Nu+1:Nu+Nv)  =  Omv.*(vhn-v2) + dt*(a21*kv1 + a22*kv2 - c2*(Gy*pn + y_py_2));
...
% approximate T^-1
Id     = speye(Nu+Nv);
T      = Id + (Id - CD) + dt*(Id-CD)^2 + dt^2*(Id-CD)^3;

% make the computed U1 and U2 divergence free; these poisson equations are decoupled
V1     = [u1;v1];
f      = 1/(c1*dt)*(M*V1 + yM1);
pressure_poisson;
...
f      = 1/(c2*dt)*(M*V2 + yM2);
pressure_poisson;
V2     = V2 - c2*dt*Om_inv.*(G*dp);

% update to u^(n+1)
V       = Vn + b1*(a_inv_11*(V1-Vn) + a_inv_12*(V2-Vn)) + b2*(a_inv_21*(V1-Vn) + a_inv_22*(V2-Vn));
uh      = V(1:Nu);
vh      = V(Nu+1:Nu+Nv);
...
% with this formulation we don't have to recompute the right hand side with
