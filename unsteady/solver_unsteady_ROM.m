...
%% for operator inference, use different snapshot data for constructing ROM basis
...
if options.rom.opinf_type ~= "intrusive" 
    if exist("opinf_snapshot_datas",'var')
        ...
        for jj = 1:numel(opinf_snapshot_datas)
            ...
            A_comb = [A_comb A];
            A_dot_comb = [A_dot_comb A_dot];
        end
        options.rom.A = A_comb;
        options.rom.A_dot = A_dot_comb;

    elseif exist("opinf_snapshot_data",'var')
        ...
        %% test
        norm(B*(B'*(Om.*opinf_V_svd))-opinf_V_svd)
    end
end
%%
