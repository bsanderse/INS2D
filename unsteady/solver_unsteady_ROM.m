...
%% load snapshot data
if (j==1) || options.rom.changing_snapshotdata
    disp('loading data...');
    ...
    X_h = pro_X_h(:,snapshot_sample); % why parallelism with line 39?
...
% % %     figure
% % %     yyaxis left
% % %     semilogy(Sigma_bc/Sigma_bc(1),'s','displayname', "singular values of y_{bc} snapshots");
% % %     ylabel("\sigma_j/\sigma_1")
% % %     
% % %     PODenergy = cumsum(Sigma_bc);
% % %     relPODenergy = PODenergy/PODenergy(end);
% % %     yyaxis right
% % %     plot(relPODenergy,'rx','displayname', "POD snapshot energy")
% % % 
% % %     legend('show')
% % %     xlabel("mode index")
% % %     ylabel("\sigma_j/\sigma_1")
        ...
        %% analysis
            if options.rom.rom_bc == 2
                X_hom = X_h - X_inhom;
                    ...
                    [U_hom,S_hom,V_hom] = svd(X_hom);
                    Sigma_hom = diag(S_hom);
                    figure
                    semilogy(Sigma_hom/Sigma_hom(1),'s','displayname', "singular values of V_{hom} snapshots");
                    PODenergy = cumsum(Sigma_hom);
                    relPODenergy = PODenergy/PODenergy(end);
                    yyaxis right
                    plot(relPODenergy,'rx','displayname', "POD snapshot energy")
                    ylabel("% POD energy")
...
%     Conv_OpInf_r = reduced_convection_operator(Conv_OpInf);
    Conv_OpInf_r = Conv_OpInf;

    norm(Diff_intrusive - Diff_OpInf)
    norm(reduced_convection_operator(Conv_intrusive)-Conv_OpInf_r)
end
...
while(n<=nt)
    ...
    %%
    if options.rom.rom_bc ~= 0 && options.rom.BC_DEIM == 0 
