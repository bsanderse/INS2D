...
% % %     else
% % %         X_bc = get_bc_vector_yBC(0,options);
% % %         U_bc = X_bc/norm(X_bc);
% % %         %%
% % %         phi_bc = U_bc;
% % %         yBC = phi_bc;
% % %         Y_M = get_yM(options,yBC);
% % %                 L = options.discretization.A;
% % %         Gx   = options.discretization.Gx;
% % %         Gy   = options.discretization.Gy;
% % %         G = [Gx;Gy];
...
% % %         tilde_phi_inhom = -Om_inv.*(G*(L\Y_M));
% % %         [Q_inhom,R_inhom] = qr(sqrt(Om).*tilde_phi_inhom); %alternative: take first vec of tilde phi inhom
% % %         phi_inhom = sqrt(Om_inv).*Q_inhom(1,:);
% % %         R_inhom = - R_inhom(1);
% % %         options.rom.phi_bc = phi_bc;
% % %         options.rom.phi_inhom = phi_inhom;
% % %         options.rom.R_inhom = R_inhom;
        %%
%         options.rom.phi_bc = U_bc;
%         options.rom.phi_inhom = options.rom.Vbc;
%         options.rom.R_inhom = 1;
        
%         no_steps = numel(t_start:dt:t_end);
% %         a_bc = phi_bc'*X_bc;
% %         a_inhom = R_inhom*a_bc;
%         a_bc = 1;
%         a_inhom = 1;
% 
%         options.rom.a_bc_matrix = a_bc*ones(1,no_steps);
%         options.rom.a_inhom_matrix = a_inhom*ones(1,no_steps);
% % %     end
end
