...
a0 = A_raw(:,1);
    ...
    diff_symm = zeros(NMs,n_rom_types,2);
    diff_largest_eig = zeros(NMs,n_rom_types,2);
    diff_largest_mag_eig = zeros(NMs,n_rom_types,2);
...
for M_ = Ms
    ...
    for j = 1:2
        for i = 1:n_rom_types
            ...
            dt_factor = 1; % default 1
            avg_rel_state_errors(M_,i,j) = average_relative_state_error(Diff,Conv,a0s(1:M_,:),options.time.dt,dt_factor*size(A_raw,2),V_snapshots,basis(:,1:M_),skip);
            ...
            diff_symm(M_,i,j) = norm(Diff-Diff');
            diff_largest_eig(M_,i,j) = max(eig(Diff));
            diff_largest_mag_eig(M_,i,j) = max(abs(eig(Diff)));
...
for i =1:n_rom_types
    ...
    for j =1:2
        ...
        figure(6)
        semilogy(diff_symm(:,i,j),'d-','DisplayName',labels_combined(i,j))
        hold on
        title("diffusion symmetry error")
        legend("show")

        figure(7)
        semilogy(diff_largest_eig(:,i,j),'d-','DisplayName',labels_combined(i,j))
        hold on
        title("diffusion largest eigen value")
        legend("show")

        figure(8)
        semilogy(diff_largest_mag_eig(:,i,j),'d-','DisplayName',labels_combined(i,j))
        hold on
        title("diffusion largest mag eigen value")
        legend("show")
    end
end
