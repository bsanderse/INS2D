%% for now, only implicit midpoint
...
tn12 = tn + 1/2*dt; 
[~,~,dF] = F_ROM_split(0*Rn,Rn,[],tn12,options,1); 
Rn12 = (eye(M) - 1/2*dt*dF)\Rn;
R = Rn + dt*dF*Rn12;


% %% general linearly implicit Runge-Kutta method for ROM
% 
...
% [~,F_rhs,~]  = F_multiple_ROM_split(Rj,Rn,[],tj,options,0);
