% Lobatto IIIC-E method of order 3
% solution of saddlepoint system,
% nonlinear_Newton iteration

% solve for U_i

lob_type = type_list(jj); % 0: Lob IIIC, 1: Lob IIIE, 2: Lob IIICE
...
if (n==2 || nonlinear_startingvalues==0)
    ...
    u2_temp     = coeff(1,2)*uhn + coeff(2,2)*uh;
    v1_temp     = coeff(1,1)*vhn + coeff(2,1)*vh;
    v2_temp     = coeff(1,2)*vhn + coeff(2,2)*vh;
    p1_temp     = coeff(1,1)*pn + coeff(2,1)*p;
    p2_temp     = coeff(1,2)*pn + coeff(2,2)*p;
    u1 = u1_temp;
    u2 = u2_temp;
    v1 = v1_temp;
    v2 = v2_temp;
...
if (lob_type==1)
    psi1    = a_inv_11*p1 + a_inv_12*p2;
    psi2    = a_inv_21*p1 + a_inv_22*p2;    
...
if ( (lob_type==1 && BC_unsteady == 1) || lob_type==2 )
