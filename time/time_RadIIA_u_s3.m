...
% RK coefficients
A_RK = [(88-7*sqrt(6))/360 (296-169*sqrt(6))/1800 (-2+3*sqrt(6))/225;...
        (296+169*sqrt(6))/1800 (88+7*sqrt(6))/360 (-2-3*sqrt(6))/225;...
        (16-sqrt(6))/36 (16+sqrt(6))/36 1/9];
A_RK_inv = inv(A_RK);

a11    = A_RK(1,1); a12    = A_RK(1,2); a13    = A_RK(1,3);
a21    = A_RK(2,1); a22    = A_RK(2,2); a23    = A_RK(2,3);
a31    = A_RK(3,1); a32    = A_RK(3,2); a33    = A_RK(3,3);
...
a_inv_11 = A_RK_inv(1,1); a_inv_12 = A_RK_inv(1,2); a_inv_13 = A_RK_inv(1,3);
a_inv_21 = A_RK_inv(2,1); a_inv_22 = A_RK_inv(2,2); a_inv_23 = A_RK_inv(2,3);
a_inv_31 = A_RK_inv(3,1); a_inv_32 = A_RK_inv(3,2); a_inv_33 = A_RK_inv(3,3);
...
% psi1    = a_inv_11*c1*p1 + a_inv_12*c2*p2 + a_inv_13*c3*p3;
% psi2    = a_inv_21*c1*p1 + a_inv_22*c2*p2 + a_inv_23*c3*p3;
% psi3    = a_inv_31*c1*p1 + a_inv_32*c2*p2 + a_inv_33*c3*p3;
% p       = pn + b1* (a_inv_11*(psi1-pn) + a_inv_12*(psi2-pn) + a_inv_13*(psi3-pn)) + ...
%                b2* (a_inv_21*(psi1-pn) + a_inv_22*(psi2-pn) + a_inv_23*(psi3-pn)) + ...
%                b3* (a_inv_31*(psi1-pn) + a_inv_32*(psi2-pn) + a_inv_33*(psi3-pn));

% our approach also gives third order:
