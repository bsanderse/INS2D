...
while (error_nonlinear2 > nonlinear_acc && i2<nonlinear_maxit)
    error_nonlinear1 = 1;
    i = 1;

% solve non-linear system without divergence-free constraint
while (error_nonlinear1 > nonlinear_acc && i<nonlinear_maxit)
...
%       end
      ...
      % error in solution of non-linear system only
      error_nonlinear1 = norm(f1,inf);
      % error in total system
      error_nonlinear2 = norm(f,inf);

      i = i+1;
      
end

if (i>= nonlinear_maxit || error_nonlinear1>nonlinear_acc || isnan(error_nonlinear1))
    error(['nonlinear (inner) iterations did not converge in ' num2str(nonlinear_maxit) ' iterations']);
end
nonlinear_its2(n,i2) = i-1;

      % make V1 divergence free
      V1 = [u1; v1];
      f  = (1/dt)*(M*V1 + yM);

      pressure_poisson;

      V1 = V1 - dt*Om_inv.*(G*dp);
      u1 = V1(1:Nu);
      v1 = V1(Nu+1:Nu+Nv);
...
if (i2>= nonlinear_maxit || error_nonlinear2>nonlinear_acc || isnan(error_nonlinear1))
    error(['nonlinear (outer) iterations did not converge in ' num2str(nonlinear_maxit) ' iterations']);
end
nonlinear_its(n) = i2-1;
