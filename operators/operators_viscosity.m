...
[B1D, Btemp, ybcl, ybcr] = BC_general_stag(Npx+2,Npx,2, ...
                                      BC.k.left,BC.k.right,hx(1),hx(end));
% then map back to Nux_in+1 points (ux-faces) with Bkux
                                  
% extend to 2D
Ak_ux  = kron(speye(Nuy_in),Bkux*A1D*B1D);
ybc    = kron(kLe,ybcl) + kron(kRi,ybcr);
yAk_ux = kron(speye(Nuy_in),Bkux*A1D*Btemp)*ybc;

%% epsilon 
%in a similar way but with different boundary conditions
[B1D, Btemp, ybcl, ybcr] = BC_general_stag(Npx+2,Npx,2, ...
                                      BC.e.left,BC.e.right,hx(1),hx(end));
% extend to 2D
Ae_ux  = kron(speye(Nuy_in),Bkux*A1D*B1D);
ybc    = kron(eLe,ybcl) + kron(eRi,ybcr);
yAe_ux = kron(speye(Nuy_in),Bkux*A1D*Btemp)*ybc;
...
yAe_uy_lr = kron(speye(Npy+2),Bvux*A1D*Btemp)*ybc;            

% apply bc in y-direction
[B2D, Btemp, ybcl, ybcu] = BC_general_stag(Npy+2,Npy,2,...
                                           BC.e.low,BC.e.up,hy(1),hy(end));
...
yAe_vx_lu = kron(Buvy*A1D*Btemp,speye(Npx+2))*ybc;
...
% extend to 2D
Ak_vy  = kron(Bkvy*A1D*B1D,speye(Nvx_in));
ybc    = kron(ybcl,kLo) + kron(ybcu,kUp);
yAk_vy = kron(Bkvy*A1D*Btemp,speye(Nvx_in))*ybc;
...
[B1D, Btemp, ybcl, ybcu] = BC_general_stag(Npy+2,Npy,2, ...
                                      BC.e.low,BC.e.up,hy(1),hy(end));
% extend to 2D
Ae_vy  = kron(Bkvy*A1D*B1D,speye(Nvx_in));
ybc    = kron(ybcl,eLo) + kron(ybcu,eUp);
yAe_vy = kron(Bkvy*A1D*Btemp,speye(Nvx_in))*ybc;
